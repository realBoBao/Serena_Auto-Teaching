...
async function getHotDb() {
  if (!_hotDb) {
    _hotDb = await open({ filename: HOT_DB, driver: sqlite3.default.Database });
    await _hotDb.exec('PRAGMA journal_mode=WAL');
  }
  return _hotDb;
}
...
async function getColdDb() {
  if (!_coldDb) {
    _coldDb = await open({ filename: COLD_DB, driver: sqlite3.default.Database });
    await _coldDb.exec('PRAGMA journal_mode=WAL');
