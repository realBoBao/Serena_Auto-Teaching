  ...
  hackernews: [
    {
      ...
      test: async () => {
        try {
          const r = await fetch('https://hn.algolia.com/api/v1/search?query=test&tags=story&hitsPerPage=1', { signal: AbortSignal.timeout(5000) });
          return r.ok;
        } catch { return false; }
      },
    ...
    },
    {
      ...
      search: async (query, limit = 10) => {
        // Firebase API không hỗ trợ search — fetch top stories rồi filter
        ...
        return stories
          ...
          .filter(n => n.title.toLowerCase().includes(query.toLowerCase().split(' ')[0]));
      },
    },
  ],
  youtube: [
