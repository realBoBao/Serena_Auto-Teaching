...
class MemoryDecayManager {
  ...
  async decayCategoryAffinity(userId) {
    try {
      ...
      const fbDb = await open({ filename: fbPath, driver: sqlite3.default.Database }).catch(() => null);
      if (!fbDb) return [];
  ...
  async decayTopicStrengths(userId) {
    try {
      ...
      const pfDb = await open({ filename: pfPath, driver: sqlite3.default.Database }).catch(() => null);
      if (!pfDb) return [];
