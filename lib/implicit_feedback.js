...
class ImplicitFeedbackManager {
  ...
  /**
   * Get recent unreplied outbound links for a user.
   * Used by discord_bot to calculate dwell time on reply.
   * @param {string} userId
   * @param {number} limit — default 5
   * @returns {Array} recent outbound links without replies
   */
  _getRecentUnreplied(userId, limit = 5) {
    return dbAll(
      `SELECT id, user_id, url, category, message_id, sent_at, clicked
       FROM outbound_links
