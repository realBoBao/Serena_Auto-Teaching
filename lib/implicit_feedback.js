...
class ImplicitFeedbackManager {
  ...
  async recordClick(linkId, userId) {
    const db = await getDb();
    if (!db) return;
    await db.run('UPDATE outbound_links SET clicked = 1, click_at = ? WHERE id = ? AND user_id = ?', [_now(), linkId, userId]);
    const row = await db.get('SELECT category FROM outbound_links WHERE id = ?', [linkId]);
    if (row) {
      await db.run(`UPDATE category_affinity SET click_count = click_count + 1, last_updated = datetime('now') WHERE user_id = ? AND category = ?`, [userId, row.category]);
    }
  }
  ...
  async recordDwellTime(linkId, userId, dwellTimeMs) {
    const db = await getDb();
    if (!db) return;
    await db.run('UPDATE outbound_links SET replied = 1, reply_at = ?, dwell_time_ms = ?, reply_latency_ms = ? WHERE id = ? AND user_id = ?', [_now(), dwellTimeMs, dwellTimeMs, linkId, userId]);
    await db.run(`INSERT INTO dwell_time_log (user_id, link_id, dwell_time_ms, category) SELECT ?, ?, ?, category FROM outbound_links WHERE id = ?`, [userId, linkId, dwellTimeMs, linkId]);
    const row = await db.get('SELECT category FROM outbound_links WHERE id = ?', [linkId]);
  ...
  async recordSkip(userId, category) {
    const db = await getDb();
    if (!db) return;
    await db.run(`UPDATE category_affinity SET skip_count = skip_count + 1, last_updated = datetime('now') WHERE user_id = ? AND category = ?`, [userId, category]);
  ...
  async _getRecentUnreplied(userId, limit = 5) {
    const db = await getDb();
    if (!db) return [];
    return db.all(`SELECT id, user_id, url, category, message_id, sent_at, clicked FROM outbound_links WHERE user_id = ? AND replied = 0 ORDER BY sent_at DESC LIMIT ?`, [userId, limit]);
  }
  ...
  async getCategoryAffinity(userId) {
    const db = await getDb();
    if (!db) return [];
    return db.all('SELECT category, implicit_score, click_count, skip_count, total_sent FROM category_affinity WHERE user_id = ? ORDER BY implicit_score DESC', [userId]);
  }
  ...
  async getImplicitSignals(userId) {
    ...
    const avgDwell = await db.get('SELECT AVG(dwell_time_ms) as avg_dwell, COUNT(*) as total_replies FROM dwell_time_log WHERE user_id = ?', [userId]);
  ...
  async markOldUnclickedAsSkipped(olderThanHours = 48) {
    const db = await getDb();
    if (!db) return 0;
    const old = await db.all(`SELECT id, user_id, category FROM outbound_links WHERE clicked = 0 AND replied = 0 AND sent_at < datetime('now', ?)`, [`-${olderThanHours} hours`]);
  ...
  async _recomputeAffinity(userId, category) {
    const db = await getDb();
    if (!db) return;
    const stats = await db.get(`SELECT COUNT(*) as total, SUM(clicked) as clicks, SUM(replied) as replies, AVG(CASE WHEN reply_latency_ms > 0 THEN reply_latency_ms END) as avg_latency FROM outbound_links WHERE user_id = ? AND category = ?`, [userId, category]);
    if (!stats || stats.total === 0) return;
    const score = _computeImplicitSignal(stats.clicks > 0, stats.avg_latency);
    await db.run(`UPDATE category_affinity SET implicit_score = ?, last_updated = datetime('now') WHERE user_id = ? AND category = ?`, [score, userId, category]);
  }
  ...
  async cleanup(daysToKeep = 90) {
    const db = await getDb();
    if (!db) return;
    await db.run("DELETE FROM outbound_links WHERE sent_at < datetime('now', ?)", [`-${daysToKeep} days`]);
    await db.run("DELETE FROM dwell_time_log WHERE recorded_at < datetime('now', ?)", [`-${daysToKeep} days`]);
