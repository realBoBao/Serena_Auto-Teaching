...
class MoodStateManager {
  ...
  async recordState(userId, { state, confidence, signals }) {
    ...
    const prev = await this.getLastState(userId);
    if (prev && prev.state !== state && db) {
      await db.run(
  ...
  async getLastState(userId) {
    const db = await getDb();
    if (!db) return null;
    return db.get('SELECT state, confidence, signals, recorded_at FROM mood_history WHERE user_id = ? ORDER BY recorded_at DESC LIMIT 1', [userId]);
  }

  /** Get mood history for past N days. */
  async getStateHistory(userId, days = 7) {
    const db = await getDb();
    if (!db) return [];
    return db.all(`SELECT state, confidence, signals, hour_of_day, recorded_at FROM mood_history WHERE user_id = ? AND recorded_at > datetime('now', ?) ORDER BY recorded_at DESC`, [userId, `-${days} days`]);
  }

  /** Get dominant mood over past N days (mode of states). */
  async getDominantMood(userId, days = 7) {
    const db = await getDb();
    if (!db) return 'neutral';
    const rows = await db.all(`SELECT state, COUNT(*) as count FROM mood_history WHERE user_id = ? AND recorded_at > datetime('now', ?) GROUP BY state ORDER BY count DESC LIMIT 1`, [userId, `-${days} days`]);
  ...
  async cleanup(daysToKeep = 90) {
    const db = await getDb();
    if (!db) return;
    await db.run("DELETE FROM mood_history WHERE recorded_at < datetime('now', ?)", [`-${daysToKeep} days`]);
    await db.run("DELETE FROM mood_transitions WHERE recorded_at < datetime('now', ?)", [`-${daysToKeep} days`]);
