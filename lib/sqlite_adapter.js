...
// ── Jest ESM fix: prevent re-declaration when module is loaded multiple times ──
if (globalThis.__sqlite_adapter_loaded) {
  // Module already loaded in this process — skip re-initialization
} else {
  globalThis.__sqlite_adapter_loaded = true;
}
...
export function getDb() {
  ...
  if (DatabaseSync) {
    ...
    // Shim: cho phép code cũ dùng db.run/get/all trực tiếp
    if (!_db.run) {
