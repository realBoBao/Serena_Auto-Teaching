...
export async function migrateFromSqlite() {
  ...
  try {
    const { getAllDbRows, openDb } = await import('./sqlite_adapter.js');
    const db = await openDb();
    const rows = getAllDbRows(db, 'SELECT id, embedding, doc_id, chunk_index, chunk_text, url, project, category, metadata FROM vectors');
