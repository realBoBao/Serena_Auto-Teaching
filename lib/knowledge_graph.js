...
export async function getDb() {
  ...
  dbPromise = (async () => {
    const db = await open({ filename: KG_DB, driver: sqlite3.Database });
...
export async function upsertEntity(name, type = 'concept', description = '', metadata = {}) {
  ...
  await db.run(
    ...
    id, name, type, description, metaJson
  );
...
export async function getGraphStats() {
  const db = await getDb();
  const entities = await db.get('SELECT COUNT(*) as n FROM entities');
  const edges = await db.get('SELECT COUNT(*) as n FROM edges');
  const types = await db.all('SELECT type, COUNT(*) as count FROM entities GROUP BY type ORDER BY count DESC');
  const relations = await db.all('SELECT relation, COUNT(*) as count FROM edges GROUP BY relation ORDER BY count DESC LIMIT 10');
  return {
...
export async function clearGraph() {
  ...
  await db.run('DELETE FROM entity_aliases');
  await db.run('DELETE FROM media_nodes');
...
export async function exportGraphForVisualization() {
  const db = await getDb();
  const entities = await db.all('SELECT id, name, type, description FROM entities');
  const edges = await db.all('SELECT source_id, target_id, relation, weight FROM edges');
