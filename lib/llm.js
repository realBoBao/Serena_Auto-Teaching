...
async function _doAsk(query, { systemPrompt, temperature, maxTokens, opts }) {
  ...
  // ── 0. Groq LPU (ưu tiên #0 — ultra-low latency, <500ms for 70B models) ──
  if (opts.provider === 'auto' || opts.provider === 'groq' || !opts.provider) {
    const groqLlm = createGroqLlm({ model: opts.model, temperature, maxTokens });
    if (groqLlm) {
      ...
      if (opts.model) {
        try {
          ...
          if (answer && answer.trim()) {
            return { answer: answer.trim(), provider: 'groq', model: opts.model };
          }
        } catch (err) {
          console.warn('[LLM] Groq model lỗi:', err?.message || err);
        }
      }
      // Try fallback models
      for (const modelName of GROQ_FALLBACK_MODELS) {
        try {
          const fallbackLlm = createGroqLlm({ model: modelName, temperature, maxTokens });
