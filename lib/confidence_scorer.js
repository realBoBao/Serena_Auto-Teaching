 ...
 */
export function scoreConfidence({ question, answer, results = [], jaccardSim = 0, skipSelfCheck = false }) {
  const reasons = [];
 ...
 */
export function formatConfidenceSuffix({ score, level }) {
  if (level === 'high') return '\n\n✅ Độ tin cậy: Cao';
  if (level === 'medium') return '\n\n⚠️ Độ tin cậy: Trung bình';
  return '\n\n❌ Độ tin cậy: Thấp — nên kiểm tra lại nguồn khác';
}
